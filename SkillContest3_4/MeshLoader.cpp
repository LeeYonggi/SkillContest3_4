	...
	if (isMatCopy == false)
	{
		for (int iMaterial = 0; iMaterial < m_Materials->size(); iMaterial++)
		{
			...
			if (pMaterial->strTexture[0])
			{
				...
				if (!bFound)
				{
					wstring tempFileName = strFilename;
					int size = tempFileName.rfind(L"/");
					tempFileName = tempFileName.substr(0, size + 1);

					pMaterial->strTexture = tempFileName + pMaterial->strTexture;
					V_RETURN(DXUTFindDXSDKMediaFileCch(str, MAX_PATH, pMaterial->strTexture.c_str()));
					pMaterial->pTexture = IMAGEMANAGER->AddTexture(pMaterial->strTexture, pMaterial->strTexture)->tex;
				}
			}
		}
	}
...
HRESULT CMeshLoader::LoadGeometryFromOBJ(const WCHAR* strFileName)
{
	...
	for (; ; )
	{
		...
		else if (0 == wcscmp(strCommand, L"mtllib"))
		{
			...
			wstring tempfile = strFileName;
			int size = tempfile.rfind(L"/");
			tempfile = tempfile.substr(0, size + 1);

			strMaterialFilename = tempfile + strMaterialFilename;
		}
...
HRESULT CMeshLoader::LoadMaterialsFromMTL(const WCHAR* strFileName)
{
	...
	wstring tempFileName = strFileName;
	int size = tempFileName.rfind(L"/");
	tempFileName = tempFileName.substr(0, size);

	vector<Material*> *temp = MESHMANAGER->GetMaterialList(tempFileName, m_Materials);
	...
	for (; ; )
	{
		...
		else if (0 == wcscmp(strCommand, L"Ka"))
		{
			...
			float r, g, b;
			InFile >> r >> g >> b;
			pMaterial->mtl.Ambient = D3DXCOLOR(r, g, b, 1);
		}
		else if (0 == wcscmp(strCommand, L"Kd"))
		{
			...
			float r, g, b;
			InFile >> r >> g >> b;
			pMaterial->mtl.Diffuse = D3DXCOLOR(r, g, b, 1);
		}
		else if (0 == wcscmp(strCommand, L"Ks"))
		{
			...
			float r, g, b;
			InFile >> r >> g >> b;
			pMaterial->mtl.Specular = D3DXCOLOR(r, g, b, 1);
		}
...
void CMeshLoader::InitMaterial(Material* pMaterial)
{
	...
	pMaterial->mtl.Ambient = D3DXCOLOR(0.2f, 0.2f, 0.2f, 1);
	pMaterial->mtl.Diffuse = D3DXCOLOR(0.8f, 0.8f, 0.8f, 1);
	pMaterial->mtl.Specular = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1);
